...
def save_transcript(session_data: dict):
    """Save transcript to file and process QA evaluation."""
    ...
    try:
        # Import evaluation function from unified_server
        # This allows standalone voice_server to also work if run independently
        try:
            sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
            # Try to import from unified_server if available
            import importlib.util
            unified_server_path = os.path.join(os.path.dirname(__file__), 'unified_server.py')
            if os.path.exists(unified_server_path):
                spec = importlib.util.spec_from_file_location("unified_server", unified_server_path)
                unified_server = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(unified_server)
                
                # Call the evaluation function
                unified_server.process_and_evaluate_transcript(session_data)
            else:
                print("Warning: unified_server.py not found, skipping QA evaluation")
        except Exception as import_error:
            print(f"Note: Could not import evaluation (voice_server running standalone): {import_error}")
            # This is OK - voice_server can run standalone without evaluation
    except Exception as e:
